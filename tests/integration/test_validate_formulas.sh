...
if ! "$ROOT_DIR/scripts/validate_formulas.sh" --formula-dir "$ok_dir/Formula" >/dev/null; then
  echo "FAIL: validate_formulas should pass for valid fixtures"
  exit 1
fi

if "$ROOT_DIR/scripts/validate_formulas.sh" --formula-dir "$bad_dir/Formula" >/dev/null 2>&1; then
